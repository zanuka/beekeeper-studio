<template>
  <div class="style-wrapper">
    ...
    <util-died-modal />
    <theme-manager-modal ref="themeManagerModal" />
...
export default Vue.extend({
  ...
  async beforeDestroy() {
    ...
    window.main.removeListener(
      AppEvent.openThemeManager,
  ...
  async mounted() {
    ...
    console.log("Setting up theme manager event listener in App.vue"); // Console log for debugging
    log.debug("Setting up theme manager event listener in App.vue");

    // Register for the openThemeManager event
    window.main.on(AppEvent.openThemeManager, () => {
      console.log("Received openThemeManager event from main process"); // Console log for debugging
      log.debug("Received openThemeManager event from main process");
      this.openThemeManager();
    });
  },
  methods: {
    ...
    // Add the openThemeManager method with enhanced debugging
    openThemeManager() {
      console.log("openThemeManager method called in App.vue"); // Console log for debugging
      log.debug("openThemeManager method called in App.vue");
      if (this.$refs.themeManagerModal) {
        console.log("Theme manager modal ref found, showing modal"); // Console log for debugging
        log.debug("Theme manager modal ref found, showing modal");
        // @ts-ignore
        this.$refs.themeManagerModal.show();
      } else {
        console.error("Theme manager modal reference not found"); // Console log for debugging
        log.error("Theme manager modal reference not found");
      }
    },
  },
});
