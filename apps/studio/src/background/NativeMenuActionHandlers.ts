...
const log = rawLog.scope("NativeMenuActionHandlers");
...
export default class NativeMenuActionHandlers implements IMenuActionHandler {
  ...
  about(): void {
    app.setAboutPanelOptions({
      ...
      authors: [
        "Matthew Rathbone",
        "Gregory Garden",
        "All the wonderful Github contributors",
      ],
  ...
  contactSupport(): void {
    shell.openExternal(
      "https://docs.beekeeperstudio.io/support/contact-support/"
    );
  }
  ...
  switchLicenseState = async (
    state: Electron.MenuItem | DevLicenseState,
    win: ElectronWindow
  ) => {
  ...
  // Updated to use the ThemeHandler
  openThemeManager = (
    _menuItem: Electron.MenuItem,
    win: ElectronWindow
  ): void => {
    log.debug("openThemeManager called in NativeMenuActionHandlers");
    if (win) {
      ThemeHandler.openThemeManager(win);
    }
  };
}
