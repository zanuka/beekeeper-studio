import {
  ...
  TransportUserSetting,
  UserSettingValueType,
  setValue,
} from "../../../common/transport/TransportUserSetting";
...
const SettingStoreModule: Module<State, any> = {
  ...
  mutations: {
    replaceSettings(state, newSettings: TransportUserSetting) {
      ...
      state.settings = _.mapValues(
        grouped,
        (v) => v[0]
      ) as unknown as IGroupedUserSettings;
    },
  ...
  actions: {
    async initializeSettings(context) {
      ...
      // Initialize theme manager with the current theme
      const themeManager = await import(
        "../../../lib/themes/ThemeManager"
      ).then((m) => m.default);
      await themeManager.initialize();
      const activeTheme = context.state.settings.theme?.value || "dark";
      await themeManager.setActiveTheme(String(activeTheme));
    },
    ...
    async save(context, { key, value }) {
      if (!key || !value) return;
      const setting =
        context.state.settings[key] ||
        (await Vue.prototype.$util.send("appdb/setting/new"));
      if (_.isBoolean(value)) setting.valueType = UserSettingValueType.boolean;
      setValue(setting, value);
      setting.key = key;
      const newSetting = await Vue.prototype.$util.send("appdb/setting/save", {
        obj: setting,
      });
      _.merge(setting, newSetting);
      context.commit(M.ADD, setting);

      // If theme setting changed, update the theme
      if (key === "theme") {
        const themeManager = await import(
          "../../../lib/themes/ThemeManager"
        ).then((m) => m.default);
        await themeManager.setActiveTheme(value);
      }
    },
  },
  getters: {
    ...
    themeValue(state, _getters, _rootState, rootGetters) {
      ...
      if (
        rootGetters.isCommunity &&
        ["system", "dark", "light"].includes(theme as string)
      ) {
        return theme;
      }
    ...
    userKeymap(state) {
      ...
      return value &&
        config.defaults.keymapTypes.map((k) => k.value).includes(value as "default" | "vim")
        ? value
        : "default";
    },
    sortOrder(state) {
